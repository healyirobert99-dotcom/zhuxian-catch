...
4. 围绕板块 beta 策略，给出 ETF / 中军载体、仓位节奏、回撤止损、持有窗口等操作复核框架。
...
> 经过连续观察和复盘，找到市场环境良好时出现有延续性的主线板块，并优先通过 ETF / 中军载体参与板块 beta。
...
```text
scripts/run_daily_review_manual.py
```
...
- 不覆盖原处理动作；
- 只增加复核优先级；
- 具体操作仍需结合 ETF / 中军载体、环境分、偏离度和回撤阈值复核。
...
```text
新闻/政策/研报/公告标题只做催化解释层和解读辅助层，不改变 A/B/C/退潮评级。
```
...
```text
scripts/sync_catalysts.py
```

手动严格生成日报时会先尝试同步最近 5 天标题。催化同步失败不阻断行情日报，但必须在终端报告失败来源；日报会继续使用本地已有催化缓存。
...
- 早期信号 + 催化增强 + ETF/中军可承接：优先解读；
- 价格强但催化弱：资金先行，等待政策/产业逻辑补强；
- 文本热但价格弱：叙事先行，不放大解读；
- 退潮行业仍有研报热度：警惕叙事滞后；
...
- 手动严格生成会补齐当日估值快照；
- 若当日 Tushare 未返回完整数据，系统不生成日报。

## 7. 手动运行规则

当前不启用自动运行。需要日报时，由用户或助手手动执行：
...
`run_daily_review_manual.py` 是严格模式；当日数据未完整发布时会失败，这是预期行为。
